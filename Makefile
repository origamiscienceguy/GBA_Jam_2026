...
# Name of your ROM
PROJECT		:= Forever_Witch_Fighter_GBA
...
SOURCE_DIRS	:= source := include := source/irq_handling := data/music := data/sfx := data/graphics := data/animations := source/scripts
SOURCE_EXTS	:= c := s := S

# Include directories
INCLUDES	:= include := source/irq_handling := source := data/music := data/sfx := data/graphics
